...
export default function PortfolioBuilder() {
  ...
  const loadLocalProjects = () => {
    // Récupérer les projets depuis localStorage si disponible
    const storedProjects = JSON.parse(localStorage.getItem('portfolioProjects') || '[]');
    
    // Si aucun projet n'est stocké, utiliser des exemples
    if (storedProjects.length === 0) {
      ...
      setPortfolioData(prevData => ({
        ...prevData,
        selectedProjects: defaultProjects.filter(p => p.featured).map(p => p.id)
      }));
      
      // Stocker les projets par défaut dans localStorage
      localStorage.setItem('portfolioProjects', JSON.stringify(defaultProjects));
    } else {
      setProjects(storedProjects);
      setPortfolioData(prevData => ({
        ...prevData,
        selectedProjects: storedProjects.filter(p => p.featured).map(p => p.id)
      }));
    }
  };
  ...
  const savePortfolio = async () => {
    try {
      // Sauvegarder les données du portfolio via API
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Créer votre portfolio</h2>
