...
export default async function ModulePage({ params }) {
  ...
  try {
      ...
      if (res.status === 404) {
        return (
          <div className="text-center py-12">
            <h1 className="text-2xl font-bold text-red-600 mb-4">Module non trouvé</h1>
        ...
        throw new Error(`Erreur lors de la récupération du module: ${res.status}`);
      }
    }
            ...
            {module.lessons.map((lesson, index) => (
              <div key={lesson.id} className="border border-gray-200 rounded-lg p-4 hover:border-indigo-300 transition-colors">
                <div className="flex items-center">
                  ...
                  <Link href={`/lessons/module/${moduleId}/lesson/${lesson.id}`} className="btn-primary text-sm py-1 px-3">
                    Commencer
                  </Link>
                </div>
              </div>
    ...
    return (
      <div className="text-center py-12">
        <h1 className="text-2xl font-bold text-red-600 mb-4">Erreur lors du chargement du module</h1>
        <p className="mb-6">Une erreur inattendue s'est produite.</p>
        <Link href="/lessons" className="btn-primary">
          Retour aux modules
        </Link>
      </div>
